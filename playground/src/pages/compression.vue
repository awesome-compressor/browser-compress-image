...
interface ImageItem {
  ...
  compressedResults?: any // 压缩结果，用于格式转换
}
...
function canConvertFormat(item: ImageItem): boolean {
  return getOptimalFormats(item.file).length > 0
}
        ...
        <div class="images-grid">
          ...
          >
            ...
            <div class="image-actions">
              ...
              <button
                v-if="canConvertFormat(item) && (item.compressedUrl || item.compressionError)"
                class="action-btn-small convert-format"
                title="Format conversion"
                @click.stop="openFormatConverter(item)"
              >
                <el-icon>
                  <Picture />
                </el-icon>
