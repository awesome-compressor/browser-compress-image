...
> 注意：这里的“最小 / 平衡 / 高质量”是相对工具集合的概念，不是对最终产物大小的固定承诺。当前发布包仍以共享根入口为主，真实 bundle 结果需要结合你的 bundler 分析。
...
### 2. 在自定义 Worker 中运行工具（实验性）

如果你要把压缩逻辑放进自己的 Worker 脚本，请把这条路径视为自定义集成方案单独验证；库内置的 Worker 管理仍处于实验阶段。
...
| 配置       | 相对体积 | 包含工具                         | 适用场景       |
| ---------- | -------- | -------------------------------- | -------------- |
| 最小配置   | 最小     | Canvas                           | 移动端、博客   |
| 基础配置   | 较小     | Canvas + CompressorJS            | 企业官网       |
| 平衡配置   | 中等     | 上述 + Browser-Image-Compression | Web 应用       |
| 高质量配置 | 较大     | 上述 + JSQuash                   | 图片处理应用   |
| 完整配置   | 最大     | 所有工具                         | 专业图片编辑器 |
